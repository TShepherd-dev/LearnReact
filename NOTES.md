...
- Lessons so far: 0001 = SFC → function component (Home.vue); 0002 = lifecycle → useEffect (MyTraining.vue, quiz 3/3); 0003 = props flow / lifting state (pagination.vue → assignment.vue, quiz 3/3); 0004 = controlled inputs (inputText.vue → membersAddUserModal.vue, quiz 3/3); 0005 = app shell (main.js+appMounter+frontend.vue+routers/core.js → createRoot+ShellLayout+Routes, quiz 3/3); 0006 = state management (store/index.js + store/shell.js → useReducer+Context, Home.vue map-helpers → useShell() hooks). Planned: 0007 = TypeScript intro (JSX→TSX migration, fluid timing).
